...
```html
<circle cx="103.71" cy="121.22" r="25.234" fill-opacity="0" stroke="#cf1d35" 
  stroke-width=".26458"/>
```
...
```bash
java -jar bin/norma-0.5.0-SNAPSHOT-jar-with-dependencies.jar --project corpus-raw  
  --fileFilter '.*/(.*).pdf' --makeProject '(\1)/fulltext.pdf'
```
...
```{r vector-fig4, echo=FALSE, out.width='100%', fig.cap="The original depiction of a funnel plot [left, reproduced under CC BY license from 10.1186/s13027-016-0058-9] and the manually extracted part that is subsequently ingested into the norma software (right)."}
...
```bash
java -jar bin/norma-0.5.0-SNAPSHOT-jar-with-dependencies.jar --project corpus-clipped 
  --fileFilter "^.*figures/figure(\\d+)/figure(_\\d+)?\\.svg"  --outputDir corpus-clipped
