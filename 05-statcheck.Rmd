...
<!-- http://www.mdpi.com/2306-5729/1/3/14/htm -->
...
kable(df, caption = 'An overview of the publishers included accompanied by descriptive statistics per publisher regarding the extracted APA results.', booktabs = TRUE) %>%
  kable_styling(latex_options = c('scale_down', 'striped', 'hold_position'), position = 'left')
...
For each of the 688,112 results, 20 variables are included, of which seven pertain to article metadata and 13 pertain to the individual statistical results. Table `r tabnr` lists all variables included in the dataset. Two specific sets of variables are worth explaining further. First, only $F$-values have two degrees of freedom (i.e., $df1$ and $df2$). For $t$-values, the reported degrees of freedom are $df2$, because $t^2(df)=F(1,df)$. For all other test statistics that include degrees of freedom, they are included in $df1$ (i.e., $\chi^2$,$r$; $Z$ contains no degrees of freedom). Second, the variable `DecisionError` indicates whether an error results in wrongly concluding statistical significance (report $p<0.05$ whereas the recalculated $p$-value yields $p>0.05$, or vice versa). If the variables `OneTail` and `OneTailedInTxt` are `TRUE` (see Table `r tabnr`), a decision error is reverted to `FALSE`.

```{r echo = FALSE}
df <- read.csv('assets/tables/statcheck-2.csv')
kable(df, caption = 'Variables included in the dataset and a description of each variable. ', booktabs = TRUE) %>%
...
The data were collected in five steps: (i) collect journal lists; (ii) spider journal pages for articles; (iii) download articles; (iv) add article metadata; and (v) mine articles for statistical results. These five steps are specified below. All code and version history is available at [https://github.com/chartgerink/2016statcheckdata](https://github.com/chartgerink/2016statcheckdata) (preserved at [http://doi.org/10.5281/zenodo.59818](http://doi.org/10.5281/zenodo.59818)). Figure `r fignr` gives a flowchart of the different steps in the data collection process.

```{r echo = FALSE, fig.cap = 'Flowchart of the data collection process, specified per step in the collection process.'}
include_graphics('assets/figures/statcheck-fig1.png')
fignr <- fignr + 1
```
